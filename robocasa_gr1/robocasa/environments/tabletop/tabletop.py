...
from robosuite.utils.errors import RandomizationError, SimulationError
...
class Tabletop(ManipulationEnv, metaclass=TabletopEnvMeta):
    ...
    def __init__(
        ...
        print(f"[DEBUG Tabletop.__init__] layout_and_style_ids={layout_and_style_ids}, layout_ids={layout_ids}, style_ids={style_ids}")
        if layout_and_style_ids is not None:
            ...
            print(f"[DEBUG Tabletop.__init__] Using provided layout_and_style_ids={self.layout_and_style_ids}")
        else:
            layout_ids = SceneRegistry.unpack_tabletop_layout_ids(layout_ids)
            style_ids = SceneRegistry.unpack_style_ids(style_ids)
            print(f"[DEBUG Tabletop.__init__] Unpacked layout_ids={layout_ids}, style_ids={style_ids}")
            self.layout_and_style_ids = [(l, s) for l in layout_ids for s in style_ids]
            print(f"[DEBUG Tabletop.__init__] Created layout_and_style_ids={self.layout_and_style_ids}")
        ...
        print(f"[DEBUG Tabletop] VALID_LAYOUTS={self.VALID_LAYOUTS}, layout_and_style_ids={self.layout_and_style_ids}")
        assert len(self.layout_and_style_ids) > 0

        print(f"[DEBUG Tabletop.__init__] Received generative_textures={generative_textures}, type={type(generative_textures)}")
        assert generative_textures in [None, False, "100p"]
        self.generative_textures = generative_textures
        print(f"[DEBUG Tabletop.__init__] Set self.generative_textures={self.generative_textures}")
    ...
    def _load_model_monitor_wrapper(func):
        ...
        Allows up to MAX_LOAD_MODEL_RETRIES recursive calls for fixture placement retry,
        but raises an error if exceeded to prevent infinite loops.
        """
        MAX_LOAD_MODEL_RETRIES = 10  # Allow retries for fixture placement (layout/style resampling)

        def wrapper(self, *args, **kwargs):
            if not hasattr(self, "_load_model_count"):
                self._load_model_count = 0
            if not hasattr(self, "_load_model_error_logged"):
                self._load_model_error_logged = False
            self._load_model_count += 1

            # Log error only once when count exceeds allowed retries and terminate simulation
            if self._load_model_count > MAX_LOAD_MODEL_RETRIES and not self._load_model_error_logged:
                error_msg = (
                    f"_load_model has been called {self._load_model_count} times! "
                    f"Exceeded maximum retries ({MAX_LOAD_MODEL_RETRIES}). "
                    "This may indicate a fixture placement issue with the current layout/style."
                )
                logging.error(error_msg)
                self._load_model_error_logged = True
                # Terminate simulation by raising an exception
    ...
    def _load_model(self, reload=True):
        ...
        elif hasattr(self, '_deterministic_layout_id') and self._deterministic_layout_id is not None:
            self.layout_id = self._deterministic_layout_id
            # Also set in _ep_meta for consistency
        ...
        # Set style_id
        print(f"[DEBUG _load_model] Checking style_id: _ep_meta has style_id={'style_id' in self._ep_meta}, _deterministic_style_id={getattr(self, '_deterministic_style_id', 'NOT_SET')}")
        if "style_id" in self._ep_meta:
            self.style_id = self._ep_meta["style_id"]
            print(f"[DEBUG _load_model] Using style_id from _ep_meta: {self.style_id}")
        elif hasattr(self, '_deterministic_style_id') and self._deterministic_style_id is not None:
            self.style_id = self._deterministic_style_id
            # Also set in _ep_meta for consistency
            self._ep_meta["style_id"] = self._deterministic_style_id
            print(f"[DEBUG _load_model] Using _deterministic_style_id: {self.style_id}")
        else:
            _, style_id = self.rng.choice(self.layout_and_style_ids)
            self.style_id = int(style_id)
            print(f"[DEBUG _load_model] Randomly chose style_id: {self.style_id} from {self.layout_and_style_ids}")

        # Always print for debugging
        print(f"[DEBUG _load_model] layout_id={self.layout_id}, style_id={self.style_id}, layout_and_style_ids={self.layout_and_style_ids}")
        if macros.VERBOSE:
    ...
    def edit_model_xml(self, xml_str):
        ...
        print(f"[DEBUG edit_model_xml] generative_textures={self.generative_textures}, condition={(self.generative_textures is not None) and (self.generative_textures is not False)}")
        if (self.generative_textures is not None) and (
            self.generative_textures is not False
        ):
            # sample textures
            assert self.generative_textures == "100p"
            print(f"[DEBUG edit_model_xml] Applying random textures with rng state")
