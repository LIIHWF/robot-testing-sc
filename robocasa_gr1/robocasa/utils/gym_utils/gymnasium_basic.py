...
def create_env_robosuite(
    ...
    # Add robocasa-specific parameters if provided
    # Note: We check for 'is not None' to allow False/0 values to be passed
    print(f"[DEBUG create_env_robosuite] Received: style_ids={style_ids}, generative_textures={generative_textures}")
    if obj_instance_split is not None:
        env_kwargs["obj_instance_split"] = obj_instance_split
    if generative_textures is not None:  # False is valid, only None is skipped
        env_kwargs["generative_textures"] = generative_textures
        print(f"[DEBUG create_env_robosuite] Added generative_textures={generative_textures} to env_kwargs")
    if layout_and_style_ids is not None:
        env_kwargs["layout_and_style_ids"] = layout_and_style_ids
    if layout_ids is not None:
        env_kwargs["layout_ids"] = layout_ids
    if style_ids is not None:  # Empty list [] is valid, only None is skipped
        env_kwargs["style_ids"] = style_ids
        print(f"[DEBUG create_env_robosuite] Added style_ids={style_ids} to env_kwargs")
...
class RoboCasaEnv(gym.Env):
    def __init__(
        ...
        **kwargs,  # Accept additional kwargs
        ...
        print(f"[DEBUG RoboCasaEnv] Calling create_env_robosuite with kwargs: style_ids={kwargs.get('style_ids')}, generative_textures={kwargs.get('generative_textures')}")
        ...
        print(f"[DEBUG RoboCasaEnv] create_env_robosuite returned, env_kwargs contains: style_ids={self.env_kwargs.get('style_ids')}, generative_textures={self.env_kwargs.get('generative_textures')}")
