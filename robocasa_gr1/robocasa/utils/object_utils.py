...
def check_obj_in_receptacle(
    ...
    If receptacle_name is not in env.objects, try to treat it as a fixture.
    """
    obj = env.objects[obj_name]
    
    # Check if receptacle_name is an object or a fixture
    if receptacle_name not in env.objects:
        # Try to treat it as a fixture
        try:
            # Try to resolve as fixture reference
            if hasattr(env, '_resolve_fixture_reference'):
                fixture = env._resolve_fixture_reference(receptacle_name)
                if fixture is not None and hasattr(fixture, 'id'):
                    # Use obj_inside_of for fixtures
                    return obj_inside_of(
                        env=env,
                        obj_name=obj.name,
                        fixture_id=fixture.id,
                        partial_check=False
                    )
            # If fixture resolution fails, return False
            return False
        except (AttributeError, KeyError, ValueError, AssertionError):
            # If fixture resolution fails (including AssertionError from get_fixture),
            # receptacle_name is neither a valid object nor a valid fixture
            # Return False to indicate object is not in receptacle
            return False
    
    # Receptacle is an object
