...
OBJ_CATEGORIES = dict(
    ...
    bread=dict(
        ...
        objaverse=dict(
            scale=[0.80, 0.80, 1.0],
            exclude=["bread_22"],  # hole on bottom
        ),
    ),
    broccoli=dict(
        types=("vegetable"),
    ...
    cardboard_box=dict(
        ...
        lightwheel=dict(
            exclude=[
                "CardboardBox018",  # odd shape
                "CardboardBox023",  # odd shape
                "CardboardBox024",  # odd shape
                "CardboardBox030",  # odd shape
            ],
        ),
    ),
    carrot=dict(
    ...
    tiered_basket=dict(
        ...
        lightwheel=dict(
            ...
            solref=(0.004, 1),
            exclude=[
                "TieredBasket008",  # too complicated spawn region
                "TieredBasket011",  # no space on the top layer
                "TieredBasket023",  # no space on the top layer
            ],
        ),
    ),
    tiered_shelf=dict(
        ...
        #     scale=1.0,
        #     exclude=[
        #         "tiered_shelf_0",  # unstable
        #         "tiered_shelf_1",  # too high
        #         "tiered_shelf_2",  # too high
        #         "tiered_shelf_3",  # missing texture
        #         "tiered_shelf_4",  # too high
        #     ],
        #     density=5000,
        #     solimp=(0.95, 0.98, 0.001),
        #     solref=(0.004, 1),
...
def get_all_obj_cats(
    groups: List[str] = None, registries: List[str] = None, attrs: List[str] = None
) -> List[str]:
    """Get all object categories for given groups, registries, and attributes."""
    if groups is None:
        groups = OBJ_GROUPS.keys()
    ...
    res = []
    for group in groups:
        for obj_cat in OBJ_GROUPS[group]:
            if any(
                registry in OBJ_CATEGORIES[obj_cat]
                and len(OBJ_CATEGORIES[obj_cat][registry].mjcf_paths) > 0
                and all(
                    getattr(OBJ_CATEGORIES[obj_cat][registry], attr, False)
                    for attr in attrs
                )
