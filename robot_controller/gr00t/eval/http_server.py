...
class HTTPInferenceServer:
    def __init__(
        ...
        self.app.post("/reset")(self.reset_policy)
    ...
    def reset_policy(self) -> JSONResponse:
        """Reset policy state/history endpoint."""
        try:
            if hasattr(self.policy, "reset"):
                self.policy.reset()
                return JSONResponse(
                    content={"status": "ok", "message": "Policy state reset successfully"}
                )
            else:
                return JSONResponse(
                    content={
                        "status": "ok",
                        "message": "Reset called but policy does not support reset",
                    }
                )
        except Exception as e:
            logging.error(traceback.format_exc())
            raise HTTPException(status_code=500, detail=f"Error resetting policy: {str(e)}")
    ...
    def run(self) -> None:
        ...
        print("  POST /reset - Reset policy state/history")
