...
class RobotInferenceServer(BaseInferenceServer):
    ...
    def __init__(self, model, host: str = "*", port: int = 5555, api_token: str = None):
        ...
        self.model = model  # Store model reference for reset endpoint
        self.register_endpoint("get_action", model.get_action)
        self.register_endpoint(
            "get_modality_config", model.get_modality_config, requires_input=False
        )
        # Register reset endpoint if model supports it
        if hasattr(model, "reset"):
            self.register_endpoint("reset", self._handle_reset, requires_input=False)

    def _handle_reset(self) -> dict:
        """
        Handle reset request to clear policy history/state.
        """
        if hasattr(self.model, "reset"):
            self.model.reset()
            return {"status": "ok", "message": "Policy state reset successfully"}
        else:
            return {"status": "ok", "message": "Reset called but model does not support reset"}
