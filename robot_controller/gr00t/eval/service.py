...
class BaseInferenceClient:
    ...
    def call_endpoint(
        ...
        try:
            ...
            raise RuntimeError(
                f"Timeout waiting for server response on endpoint '{endpoint}'. "
                f"Server at {self.host}:{self.port} may not be running or is not responding. "
                f"Timeout was set to {self.timeout_ms}ms."
            )
