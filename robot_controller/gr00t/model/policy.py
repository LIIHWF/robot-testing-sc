...
class Gr00tPolicy(BasePolicy):
    ...
    def __init__(
        ...
        try:
            ...
            # Convert relative path to absolute path to avoid HFValidationError
    ...
    def reset(self):
        """
        Reset the policy state/history. This clears any cached state or history
        that might be maintained between inference calls.
        
        Currently, the model is stateless, but this method is provided for
        future compatibility and to allow clearing any potential caching mechanisms.
        """
        # Clear any potential cache in the model
        if hasattr(self.model, "backbone") and hasattr(self.model.backbone, "eagle_model"):
            # Clear any potential KV cache in the transformer model
            if hasattr(self.model.backbone.eagle_model, "reset_cache"):
                self.model.backbone.eagle_model.reset_cache()
